...
impl FieldElement {
    ...
    pub fn sqrt(&self) -> CtOption<Self> {
        ...
        let (status, result) = crate::call_sqrt_hook(
            value.to_bytes().as_slice(),
            Self::MODULUS,
            NQR.to_bytes().as_slice(),
        );
        ...
        if status == 0 {
            assert!(
                ((result * result).negate(1) + value * &NQR)
                    .normalizes_to_zero()
                    .unwrap_u8()
                    == 1,
    ...
    pub fn invert(&self) -> CtOption<Self> {
        ...
        assert!(
            ((result * value).negate(1) + Self::ONE)
                .normalizes_to_zero()
                .unwrap_u8()
                == 1,
            "Inv hook returned invalid hint, invert is invalid."
        );
