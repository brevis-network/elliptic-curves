...
pub(crate) fn call_sqrt_hook(x: &[u8], modulus: &'static str, nqr: &[u8]) -> (u8, Vec<u8>) {
    pico_patch_libs::unconstrained! {
        ...
        pico_patch_libs::io::write(
            pico_patch_libs::io::FD_FP_SQRT,
            buf.as_slice()
        );
    }

    let status: u8 = pico_patch_libs::io::read_vec()
        .first()
        .copied()
        .expect("sqrt hook should have a status");
    let result = pico_patch_libs::io::read_vec();

    (status, result)
}

/// Call the inverse hook.
///
...
pub(crate) fn call_inv_hook(x: &[u8], modulus: &'static str) -> Vec<u8> {
    pico_patch_libs::unconstrained! {
        ...
        pico_patch_libs::io::write(pico_patch_libs::io::FD_FP_INV, buf.as_slice());
    }
