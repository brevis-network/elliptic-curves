...
#[cfg(target_os = "zkvm")]
mod succinct_types {
    use super::{scalar, FieldElement, NistP256};
    use elliptic_curve::{subtle::CtOption, FieldBytes};

    impl pico_patch_libs::ecdsa::ECDSACurve for NistP256 {
        // a = -3
        ...
        type PicoAffinePoint = pico_patch_libs::secp256r1::Secp256r1Point;
    }

    impl pico_patch_libs::ecdsa::Field<NistP256> for FieldElement {
    ...
    /// Elliptic curve point in affine coordinates.
    ///
    /// For use inside the zkvm.
    pub type AffinePoint = pico_patch_libs::ecdsa::AffinePoint<NistP256>;

    /// Elliptic curve point in projective coordinates.
    ///
    /// For use inside the zkvm.
    pub type ProjectivePoint = pico_patch_libs::ecdsa::ProjectivePoint<NistP256>;

    /// The actual scalar type used in the zkvm.
    pub type Scalar = scalar::Scalar;
}
